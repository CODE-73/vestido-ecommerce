...
export async function returnOrder(data: ReturnOrderSchemaType) {
  ...
  const result = await prisma.$transaction(async (prisma) => {
    ...
    await Promise.all(
      validatedData.returnDetails.map(async (returnDetails) => {
        const fulfillmentDetails = await prisma.fulfillment.findFirst({
          where: {
            id: returnDetails.fulfillmentId,
          },
          include: {
            fulfillmentItems: {
              select: {
        ...
        if (!fulfillmentDetails) {
          throw new VestidoError({
            name: 'ReturnOrderFulfillmentNotFoundError',
            message: 'Fulfillment Not Found Error',
            httpStatus: 404,
            context: {
              orderId: returnDetails.fulfillmentId,
            },
          });
        }
        ...
        const refundAmount = returnDetails.returnItems.reduce((sum, item) => {
        ...
        const returnOrder = await prisma.return.create({
          data: {
            fulfillmentId: returnDetails.fulfillmentId,
            orderId: validatedData.orderId,
            ...
            returnItems: {
              createMany: {
                data: returnDetails.returnItems.map((i) => ({
        ...
        const validatedMeasurements =
          ReturnPackageSchema.parse(fulfillmentDetails);
