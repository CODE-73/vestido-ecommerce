...
type SignupsWidgetProps = InjectedWidgetProps<{
  groupBy: GroupBy;
}>;

const SignupCountWidget: React.FC = () => {
  const defaultFilters = { groupBy: 'daily' };
  return (
    <WidgetWrapper widgetId="signups" defaultFilters={defaultFilters}>
      {/* @ts-expect-error Props are filled by the WidgetWrapper */}
      <SignupsWidgetDisplay />
    </WidgetWrapper>
  );
};

const SignupsWidgetDisplay: React.FC<SignupsWidgetProps> = ({
  fromDate,
  toDate,
  ...
  const { data: signup_data } = useAuthAnalytics(filter);
...
export default SignupCountWidget;
